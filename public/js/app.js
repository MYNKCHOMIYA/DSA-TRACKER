...
function renderStatsCards() {
  ...
  // Sheet progress
  const sheetPct = sheetTotal > 0 ? Math.round((totalSolved / sheetTotal) * 100) : 0;
  ...
  const avgPace = totalSolved > 0 ? (totalSolved / daysSinceStart).toFixed(1) : 0;
...
function renderTimeline() {
  ...
  if (targetDate) {
    ...
    const timeProgress = totalSpan > 0 ? Math.min(100, Math.max(0, (elapsed / totalSpan) * 100)) : 0;
    ...
    const idealSolved = totalDays > 0 ? Math.round((daysPassed / totalDays) * sheetTotal) : 0;
...
function renderCategoriesSection() {
  ...
  if (Array.isArray(repoFolders)) {
    STRIVER_SHEET.categories.forEach(cat => {
      ...
      const matchedFolders = repoFolders.filter(f =>
        f.type === 'dir' && mappedFolders.some(m =>
          f.name.toLowerCase().includes(m.toLowerCase())
        )
      );
      // If folder exists, assume some progress — this is a rough indicator
      solvedPerCategory[cat.id] = matchedFolders.length > 0 ? Math.min(cat.totalProblems, Math.max(1, Math.round(cat.totalProblems * 0.3))) : 0;
    });
  }
  ...
  if (totalManual > 0) {
    ...
    const categoriesWithFolders = STRIVER_SHEET.categories.filter(c => solvedPerCategory[c.id] > 0);
    const totalInFolders = categoriesWithFolders.reduce((s, c) => s + c.totalProblems, 0);
...
function renderEfficiency() {
  ...
  if (userSettings.targetDate) {
    const daysLeft = Math.max(1, Math.ceil((new Date(userSettings.targetDate) - today) / (1000 * 60 * 60 * 24)));
  ...
  const daysNeeded = parseFloat(currentPace) > 0 ? Math.ceil(remaining / parseFloat(currentPace)) : '∞';
...
function openSettings() {
  ...
  // Populate fields
  document.getElementById('setDailyTarget').value = userSettings.dailyTarget || 5;
  document.getElementById('setStartDate').value = userSettings.startDate ? new Date(userSettings.startDate).toISOString().split('T')[0] : '';
  document.getElementById('setTargetDate').value = userSettings.targetDate ? new Date(userSettings.targetDate).toISOString().split('T')[0] : '';
  document.getElementById('setTotalProblems').value = userSettings.totalSheetProblems || 474;
  document.getElementById('setSolvedCount').value = userSettings.manualSolvedCount || 0;
  document.getElementById('setLcUsername').value = userSettings.leetcodeUsername || '';
  document.getElementById('setGhUsername').value = userSettings.githubUsername || '';
  document.getElementById('setGhRepo').value = userSettings.githubStriverRepo || '';
}
...
async function saveSettings() {
  const payload = {
    ...
    totalSheetProblems: parseInt(document.getElementById('setTotalProblems').value),
    manualSolvedCount: parseInt(document.getElementById('setSolvedCount').value),
...
function openLogModal() {
  ...
  chipsEl.innerHTML = ALL_TOPICS.map(t =>
...
async function saveDetailedLog() {
  ...
  const topics = Array.from(document.querySelectorAll('.topic-chip.selected'))
...
function setupExternalLinks(user) {
  ...
  if (user.leetcodeUsername) {
    document.getElementById('linkLeetcode').href = `https://leetcode.com/u/${user.leetcodeUsername}/`;
  }
  if (user.githubUsername && user.githubStriverRepo) {
    document.getElementById('linkGithub').href = `https://github.com/${user.githubUsername}/${user.githubStriverRepo}`;
  }
}
