      ...
      datasets: [
        {
          ...
          borderColor: "rgba(153, 70, 54, 0.35)",
          borderDash: [6, 4],
          borderWidth: 1.5,
          fill: { target: "origin", above: "rgba(153, 70, 54, 0.05)" },
          pointRadius: 0,
          tension: 0.2,
          spanGaps: true,
        },
        {
          ...
          borderColor: "#B9D6F2",
          borderWidth: 2.5,
          fill: { target: "origin", above: "rgba(185, 214, 242, 0.06)" },
          pointRadius: 3,
          pointBackgroundColor: "#B9D6F2",
          pointBorderColor: "rgba(185, 214, 242,0.3)",
          pointBorderWidth: 4,
          pointHoverRadius: 5,
          tension: 0.35,
          spanGaps: false,
    ...
    options: {
        ...
        tooltip: {
          ...
          callbacks: {
            ...
            label: (context) => {
              ...
              if (ds === "Ideal Pace")
                return `🎯 Target: You should have solved ${val} problems`;
              if (ds === "Actual Progress")
                return `🚀 Actual: You solved ${val} problems`;
              if (ds === "Projected")
                return `📈 Projected: If you keep this pace, you'll reach ${val}`;
              return `${ds}: ${val}`;
            },
          },
        },
        annotation: {
          ...
          afterDraw: (chart) => {
            ...
            if (todayIndex !== -1) {
              ...
              ctx.lineWidth = 1.5;
              ctx.strokeStyle = "rgba(153, 70, 54, 0.5)";
              ctx.setLineDash([4, 4]);
              ctx.stroke();

              // Draw "TODAY" badge
              ctx.fillStyle = "rgba(153, 70, 54, 0.9)";
      ...
      scales: {
        x: {
          ...
          ticks: { maxTicksLimit: 8, font: { size: 13 }, color: "#6b6280" },
        },
        y: {
          beginAtZero: true,
          grid: { color: "rgba(255,255,255,0.03)" },
          border: { display: false, dash: [4, 4] },
          ticks: { font: { size: 13 }, color: "#6b6280" },
        },
      },
    },
    plugins: [
...
function renderCategoryBars(categories, solvedPerCategory) {
  ...
  container.innerHTML = categories
    .map((cat) => {
      ...
      else if (pct >= 50) color = "#B9D6F2";
      else if (pct >= 20) color = "#CBA135";
      else color = "#994636";
...
function renderHeatmap(calendarData) {
  ...
  while (current <= today) {
    ...
    cells.push(
      `<div class="heatmap-cell ${level}" title="${dateStr}: ${count} submission${count !== 1 ? "s" : ""}"></div>`,
    );
...
function updateCircularProgress(done, target) {
  ...
  if (motEl) {
    if (done === 0) {
      motEl.className = "today-motivation start";
      motEl.textContent = "Let's go!";
    } else if (pct < 50) {
      motEl.className = "today-motivation start";
      motEl.textContent = "Keep going!";
    } else if (pct < 100) {
      motEl.className = "today-motivation half";
      motEl.textContent = "Halfway there!";
    } else {
      motEl.className = "today-motivation done";
      motEl.textContent = "Crushed it!";
    }
  }
}
    ...
    for (let i = dates.length - 1; i >= 0; i--) {
      ...
      rowsHtml += `
        <tr style="border-bottom: 1px solid rgba(255,255,255,0.05);">
          ...
          <td style="padding: 12px 8px; text-align: right; color: #994636;">${st}</td>
