...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('lcUsername').addEventListener('blur', verifyLeetCode);
});
...
async function verifyLeetCode() {
  ...
  verifyEl.innerHTML = '<span class="verify-badge loading">⏳ Verifying...</span>';
  ...
  try {
    ...
    if (data && data.username) {
      ...
      verifyEl.innerHTML = '<span class="verify-badge error">❌ Username not found</span>';
      verifiedLeetcode = false;
    }
  } catch (err) {
    verifyEl.innerHTML = '<span class="verify-badge error">⚠️ Could not verify (API may be slow)</span>';
  }
}
...
function updateTargetPreview() {
  ...
  const total = parseInt(document.getElementById('totalProblems')?.value) || 474;
...
function fillSummary() {
  ...
  const lcUser = document.getElementById('lcUsername').value.trim() || 'Not set';
  const ghUser = document.getElementById('ghUsername').value.trim() || 'Not set';
  const ghRepo = document.getElementById('ghStriverRepo').value.trim() || 'Not set';
  const target = document.getElementById('dailyTarget').value;
  const startDate = document.getElementById('startDate').value;
  const targetDate = document.getElementById('targetDate').value || 'Auto-calculate';
...
async function completeOnboarding() {
  ...
  const payload = {
    ...
    totalSheetProblems: parseInt(document.getElementById('totalProblems').value) || 474,
    manualSolvedCount: parseInt(document.getElementById('solvedCount').value) || 0,
  };
